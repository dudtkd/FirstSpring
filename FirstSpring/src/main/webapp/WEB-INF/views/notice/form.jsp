				...
				<div class="">
					<div class="card-body">
						<form method="post" action="/notice/insert.do" id="noticeForm" class="form-horizontal">
							<c:if test="${status eq 'u' }"> <!-- 수정을 클릭하여 넘어오면 status에 'u'를 담아서 넘어오게 된다 -->
								<input type="hidden" name="noticeNo" value="${notice.noticeNo }">
							</c:if>
							<div class="form-group row">
								<label class="col-sm-2 control-label" >제목</label>
								<div class="col-sm-10">
									<input name="noticeTitle" type="text" id="noticeTitle" class="form-control" value="${notice.noticeTitle }" placeholder="공지사항을 입력해주세요.">
								</div>
							...
							<div class="form-group row mt-4">
								...
								<div class="col-sm-10">
									<textarea name="noticeContent" cols="50" rows="5" id="noticeContent" class="form-control" placeholder="content">${notice.noticeContent }</textarea>
								</div>
							...
							<div class="form-group row mt-4">
								<div class="col-sm-offset-2 col-sm-12 ">
									...
									<c:if test="${status eq 'u' }"> <!-- 수정을 눌러서 넘어오면 취소를 보여주고 -->
										<input type="button" value="취소" class="btn btn-danger float-right" id="cancelBtn">
									</c:if>
									<c:if test="${status ne 'u' }"> <!-- 등록을 눌러서 넘어오면 목록을 보여준다. -->
...
$(function(){
	...
	addBtn.on("click",function(){
		var title = $("#noticeTitle");	// 제목 값 가져오기
		/* CKEDITOR 사용시 이 방법으로 값을 가져와야 한다. */
		var content = CKEDITOR.instances.noticeContent.getData();	// 내용 값 가져오기
	
		if(title == null || title == ""){ // 제목이 널값이거나 공백이면 입력하지 않은상태
			alert("제목을 입력해주세요");
			$("#noticeTitle").focus();
			return false;
		}
	...
	cancelBtn.on("click",function(){
		loaction.href="/notice/detail.do?noticeNo="${notice.noticeNo};
	});
	// 목록 버튼 이벤트	
