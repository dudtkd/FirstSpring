				...
				<div class="">
					<div class="card-body">
						<form method="post" action="/board/insert.do" id="boardForm" class="form-horizontal">
							<c:if test="${status eq 'u' }"> <!-- 만약 넘어온 값이 수정으로 넘어오면 boNo를 같이 넘김 -->
								<input type="hidden" name="boNo" value="${board.boNo }"/>
							</c:if>
							<div class="form-group row">
								<label class="col-sm-2 control-label" >제목</label>
								<div class="col-sm-10">
									<input name="boTitle" type="text" id="boTitle" class="form-control" value="${board.boTitle }" placeholder="제목을 입력해주세요.">
								</div>
							...
							<div class="form-group row mt-4">
								...
								<div class="col-sm-10">
									<textarea name="boContent" cols="50" rows="5" id="boContent" class="form-control" placeholder="content">${board.boContent }</textarea>
								</div>
...
// 		CKEDITOR 사용시 값을 그대로 가져올수 없다 
		
		// CKEDITOR 내용 값 가져오기 
		var content = CKEDITOR.instances.boContent.getData();
		if(title == null || title == ""){	// 제목이 널값일때 알러트 창으 띄우고 제목쪽으로 포커스를 줌
			alert("제목을 입력해주세요");
			$("#boTitle").focus();
			return false;
		}
	...
	cancelBtn.on("click",function(){
		loaction.href="/board/detail.do?boNo=${board.boNo}";
	});
	// 목록 버튼 이벤트	
